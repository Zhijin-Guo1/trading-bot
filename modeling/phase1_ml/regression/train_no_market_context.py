#!/usr/bin/env python3
"""
Pure Text and LLM Features Regression (No Market Context)
===========================================================
This version excludes market context features (momentum, VIX) to focus 
solely on textual and LLM-generated linguistic features for prediction.

Key Changes:
- Removed momentum_7d, momentum_30d, momentum_90d, momentum_365d, vix_level
- Focus on pure text-based signals without market context
"""
...
class PureTextVolatilityRegressor:
    """Regression model using only text and LLM features (no market context)"""
    ...
    def extract_llm_features(self, df, is_training=False):
        """Extract LLM-generated features from dataframe (EXCLUDING market context)"""
        
        # Define feature groups - NO MARKET CONTEXT FEATURES
        numeric_features = [
        ...
        # NOTE: Market context features REMOVED
        # NO: momentum_7d, momentum_30d, momentum_90d, momentum_365d, vix_level
    ...
    def create_features(self, train_df, val_df, test_df):
        """Create combined TF-IDF and LLM features (no market context)"""
        
        print("\n" + "="*60)
        print("FEATURE EXTRACTION (Pure Text & LLM Only)")
        print("="*60)
        ...
        # Extract LLM features (WITHOUT market context)
        print("\n2. Extracting LLM features (no market context)...")
        ...
        print(f"  Note: Market context features (momentum, VIX) excluded")
        ...
        print(f"    - LLM features (no market): {X_train_llm.shape[1]:,}")
    ...
    def run_experiments(self, train_df, val_df, test_df):
        ...
        # Create features (no market context)
        ...
        print("\n" + "="*60)
        print("RUNNING REGRESSION EXPERIMENTS (No Market Context)")
        print("="*60)
        ...
        print("\n" + "="*60)
        print("RUNNING CLASSIFICATION EXPERIMENTS (No Market Context)")
    ...
    def analyze_results(self):
        ...
        print("\n" + "="*60)
        print("RESULTS ANALYSIS (Pure Text/LLM Features Only)")
        print("="*60)
        ...
        print("\n4. RETURNS vs VOLATILITY PREDICTION")
        print("-"*40)
        ...
        # Save results
        self.results.to_csv('regression_results_no_market.csv', index=False)
        print("\n\nDetailed results saved to: regression_results_no_market.csv")
    
    def feature_importance_analysis(self, train_df, val_df, test_df):
        """Analyze feature importance for best model"""
        
        print("\n" + "="*60)
        print("FEATURE IMPORTANCE ANALYSIS (No Market Context)")
        print("="*60)
        ...
        print("\n\nLLM Features Impact Summary (No Market Context):")
        print("-"*40)
        ...
        if hasattr(self, 'llm_feature_names') and len(self.llm_feature_names) == len(llm_coefs):
            print("\nTop 10 LLM features by importance (market features excluded):")
...
def main():
    ...
    print("="*60)
    print("PURE TEXT/LLM FEATURES REGRESSION ANALYSIS")
    print("Predicting Volatility WITHOUT Market Context Features")
    print("="*60)
    print("\nExcluded features: momentum_7d, momentum_30d, momentum_90d,")
    print("                   momentum_365d, vix_level")
    print("="*60)
    
    # Initialize regressor
    regressor = PureTextVolatilityRegressor()
    ...
    # Final summary
    print("\n" + "="*60)
    print("KEY FINDINGS (Without Market Context)")
    print("="*60)
    ...
    # Note about market context
    print(f"\n3. Market Context Impact:")
    print(f"   This analysis excludes market momentum and VIX features")
    print(f"   Results show pure textual signal strength")
    ...
    if best_result['test_correlation'] > 0.15:
        print(f"   ✓ Model shows meaningful predictive signal from text alone")
    elif best_result['test_correlation'] > 0.10:
        print(f"   ⚠ Model shows weak predictive signal from text alone")
    else:
        print(f"   ✗ Model shows very weak predictive signal from text alone")
    
    print("\n" + "="*60)
    print("ANALYSIS COMPLETE (No Market Context)")
    print("="*60)
    
    # Save final summary
    summary = {
        'timestamp': datetime.now().isoformat(),
        'note': 'Analysis excludes market context features (momentum, VIX)',
        'best_model': {
    ...
    with open('regression_summary_no_market.json', 'w') as f:
        json.dump(summary, f, indent=2)
    
    print("\nSummary saved to: regression_summary_no_market.json")
