...
def load_original_filings(years=[2014, 2015, 2016, 2017, 2018, 2019]):
    """Load all GPT summaries from the results folder"""
    all_filings = []

    for year in years:
        filing_path = f'../../data/data_processing/enhanced_eight_k/texts/merged_texts_{year}.json'
        if os.path.exists(filing_path):
            with open(filing_path, 'r') as f:
                filings = json.load(f)
                all_filings.extend(filings)
                print(f"Loaded {len(filings)} summaries from {year}")

    df_filings = pd.DataFrame(all_filings)
    print(f"\nTotal summaries loaded: {len(df_filings)}")
    return df_filings
...
def create_classification_targets(df):
    ...
    df['binary_target'] = (df['immediate_reaction_pct'] > 0).astype(int)

    # Three-class classification (UP >1%, STAY ±1%, DOWN <-1%)
    conditions = [
        df['immediate_reaction_pct'] > 1.0,
        df['immediate_reaction_pct'] < -1.0
    ]
...
def create_temporal_splits(df, val_ratio=0.1, test_ratio=0.1):
    ...
    print(
        f"Train: {train_df['filing_date'].min().date()} to {train_df['filing_date'].max().date()} ({len(train_df)} samples)")
    print(
        f"Val:   {val_df['filing_date'].min().date()} to {val_df['filing_date'].max().date()} ({len(val_df)} samples)")
    print(
        f"Test:  {test_df['filing_date'].min().date()} to {test_df['filing_date'].max().date()} ({len(test_df)} samples)")
...
def save_datasets(train_df, val_df, test_df, output_dir='next_day_full_data'):
...
def create_dataset_statistics(train_df, val_df, test_df):
    ...
    # Save statistics
    with open('next_day_full_data/dataset_statistics.json', 'w') as f:
        json.dump(stats, f, indent=2)

    print("\nDataset statistics saved to next_day_full_data/dataset_statistics.json")
...
def main():
    ...
    # df_summaries = load_original_filings()
